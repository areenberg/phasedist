...
fit = ph.fit(obs=obs,nphases=3,dtype="generlang",discrete=True,verbose=True,itermax=1000,tolerance=0.0)
phdist = fit.getdist()

# Fit a negative binomial-3 DPH distribution using matrixdist in R (installs all dependencies first)

subprocess.run(["Rscript","validation/negbinom3_in_R.R"], check=True)
matdist_result = pyreadr.read_r('validation/negbinom_ph_parameters.RData')
matdist = ph.dist(discrete=True,initdist=matdist_result['initdist'].values.flatten(),phgen=matdist_result['phgen'].values)
...
plt.title("Probability Mass Comparison")
plt.xlabel("Value")
...
file = Path("validation/negbinom_ph_parameters.RData")
file.unlink()
