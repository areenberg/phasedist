...
# Load data
obs <- read.table(file="validation/negbinom3_data.csv",header=TRUE)

# Fit the model
obj <- dph(structure = "gerlang", dimension = 3)
f <- fit(obj, obs$Samples, stepsEM = 1000, every = 100)
initdist <- f@pars$alpha
phgen <- f@pars$S

# Save the parameters
save(initdist,phgen, file = "validation/negbinom_ph_parameters.RData")